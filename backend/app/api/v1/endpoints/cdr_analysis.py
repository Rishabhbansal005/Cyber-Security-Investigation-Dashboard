...
from app.services.forensics.cdr_parser import CDRParser

router = APIRouter(prefix="/cdr", tags=["Mobile Forensics"])
logger = logging.getLogger(__name__)

@router.post("/{evidence_id}/analyze", response_model=Dict[str, Any])
async def analyze_cdr_evidence(
    evidence_id: str,
    current_user: CurrentUser = Depends(require_investigator)
):
    """Analyze an uploaded CDR CSV file."""
    try:
        ...
        # 3. Analyze CDR
        try:
            analysis_results = CDRParser.parse_cdr(temp_path, evidence["original_file_name"])
        except Exception as e:
            logger.error(f"Failed to parse CDR: {e}")
            db.table("evidence").update({"processing_status": "failed"}).eq("id", evidence_id).execute()
            raise HTTPException(status_code=500, detail="Failed to parse CDR")
        finally:
        ...
        logger.error(f"CDR analysis error for {evidence_id}: {e}")
        db.table("evidence").update({"processing_status": "failed"}).eq("id", evidence_id).execute()
