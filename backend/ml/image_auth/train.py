"""Train EfficientNet-B0: authentic vs manipulated (StyleGAN) faces.

Fits RTX 4050 6GB: batch 8, AMP, 224px.
Default data: C:\\Users\\asus_pc\\datasets\\image_auth_subset
Writes: backend/ml/artifacts/image_auth/
"""
...
DEFAULT_DATA = Path(r"C:\Users\asus_pc\datasets\image_auth_subset")
...
def make_loaders(data_dir: Path, batch_size: int, num_workers: int):
    ...
    # ImageFolder sorts folder names: authentic=0, manipulated=1
...
def high_threshold(y_true, y_prob, min_precision: float = 0.90) -> float:
    """Smallest threshold where precision(manipulated) >= min_precision; else 0.75."""
    best = 0.75
...
def main():
    ...
    batch_size = 8
    epochs = 8
    num_workers = 0  # Windows + 16GB RAM: keep 0
    print(f"device={device} amp={use_amp} data={data_dir}")
    if device.type != "cuda":
        print("WARNING: no GPU — this will be too slow for the 2-day plan.")
    ...
    metrics = {
        ...
        "history": history,
        "disclaimer": (
            "Screens synthetic/GAN faces (e.g. fake profiles). "
            "Not a morph/video-deepfake lab tool. Assistive only."
        ),
